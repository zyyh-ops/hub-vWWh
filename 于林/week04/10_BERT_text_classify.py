...
# 加载和预处理数据
dataset_df = pd.read_csv('style_classify.csv', sep=',')

# 初始化 LabelEncoder，用于将文本标签转换为数字标签
lbl = LabelEncoder()
# 拟合数据并转换前500个标签，得到数字标签
labels = lbl.fit_transform(dataset_df.iloc[:, 1].values[:7000])
# 提取前500个文本内容
texts = list(dataset_df.iloc[:, 0].values[:7000])
...
model = BertForSequenceClassification.from_pretrained(
    './assets/models/bert-base-chinese', num_labels=4  # 判别式模型，分多少类
)
...
# 提取预测标签（argmax获取最可能的类别）
pred_labels = np.argmax(predictions.predictions, axis=1)

# 输出分类结果
for text, true_label, pred_label in zip(x_test, test_labels, pred_labels):
    print(f"文本: {text[:50]}...")
    print(f"真实标签: {lbl.inverse_transform([true_label])[0]}")
    print(f"预测标签: {lbl.inverse_transform([pred_label])[0]}")
    print("-" * 50)
